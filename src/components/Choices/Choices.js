...
const Choices = ({ choices, onChoiceSelect }) => {
  ...
  const handleButtonClick = useCallback((choice, index) => {
  ...
  const handleKeyDown = useCallback((event) => {
    if (event.code === 'ArrowRight') {
      ...
      setHoveredButton((prev) => (prev - 1 + choices.length) % choices.length); // Passer au bouton précédent
  ...
  return (
    <div className={`choices-container ${fadeOut ? 'fade-out' : 'fade-in'}`}>
      {choices.map((choice, index) => (
        <button
          key={index}
          className={`choice-button ${hoveredButton === index ? 'hover' : ''} ${selectedButton === index ? 'selected' : ''}`}
